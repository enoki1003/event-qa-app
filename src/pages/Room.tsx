  ...
  if (error || !room) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4 p-4">
        ...
        <button onClick={() => navigate("/")} className="text-indigo-600 hover:underline text-sm">トップに戻る</button>
      </div>
    );
  }
            ...
            <div className="flex gap-1">
              {(["new", "old"] as const).map((o) => (
                <button key={o} onClick={() => setSortOrder(o)}
                  className={`px-2 py-1 text-xs rounded-lg transition-colors ${sortOrder === o ? "bg-indigo-100 text-indigo-700 font-medium" : "text-gray-400 hover:text-gray-600"}`}>
                  {o === "new" ? "新しい順" : "古い順"}
                </button>
              ))}
            </div>
                  ...
                  <div key={q.id} className={`px-4 py-3 ${q.isAnswered ? "opacity-60" : ""} ${isPending ? "bg-orange-50" : ""}`}>
                    ...
                    {replies.length > 0 && (
                      <div className="mt-2 space-y-1.5">
                        {replies.map((r) => (
                          <div key={r.id} className={`text-xs rounded-xl px-3 py-2 flex gap-2 ${r.isPrivate ? "bg-yellow-50 border border-yellow-100" : "bg-indigo-50 border border-indigo-100"}`}>
                            <span className="text-gray-400 flex-shrink-0">↪</span>
                            <div>
                              <span className="font-medium text-indigo-700">{replyLabel}</span>
